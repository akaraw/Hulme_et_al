...
main <- function(){
	...
	mysize <- read.table(args[5])
	mfl <- mysize[1:length(mysize)]>999
	mysize <- mysize[mfl]
	sz.min <- min(mysize)
	rm(hap, mybam)
	...
	#sz2 <- 50000
	#nr.sz
	fl <- DSFT(nr.sz, sz.min)
	DSFT.shannon <- Shannon(nr.sz[fl])
	...
	if (sz > 999){ 
		dataplus <- data.frame(base, gene, shannon_after_thr, DSFT.shannon) #sz, shannon_after_thr, ND, Mfe)
		#smpsize <- data.frame(base, sz)
	
		write.table(dataplus, file="./DSFT_Shannon.csv", append = T, sep = "\t", row.names = F, col.names = F) 
